...
Version 0.5.6 streams Hermes response drafts to Customer Map and retains edit-based streaming as a compatibility fallback. It also reports safe web-search/page-read activity and supports true task cancellation: stopping a turn cancels the matching in-flight Hermes session instead of only stopping browser polling. Ordinary Customer Map turns run on a fail-closed Hermes platform allowlist (`web`, with MCP and all local execution toolsets removed). They cannot access terminal, files, code execution, delegation, cron, or mail commands. Customer Map mail actions bypass the model and enter dedicated adapters.
