...
Version 0.4.1 executes Customer Map `sendEmail` and `saveDraft` actions directly through fixed `gog` argument mappings. HTML always uses `--body-html`, plain-text fallback always uses `--body`, and the connector never uses shell redirection, heredocs, body files, or `/dev/stdin`. It also suppresses Hermes' inapplicable `/sethome` onboarding notice on Customer Map request/response sessions while preserving ordinary Agent output, streaming, and relay behavior.
